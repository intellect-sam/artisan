        ...
        {REQUIREMENTS.map((r) => (
          <div key={r.label} className="flex items-center gap-1.5">
            <div className={cn(
              ...
              r.test(value) ? "bg-emerald-500" : "bg-muted"
            )}>
...
const STEPS = [
  ...
  { num: 2, label: "Your Profile",    desc: "Personal details" },
    ...
    <div className="hidden lg:flex lg:w-[42%] relative flex-col justify-between bg-[#1C1C1C] p-12 overflow-hidden">
      ...
      <div className="relative z-10">
        <p className="text-white/50 text-sm italic leading-relaxed">
          &ldquo;Found a reliable plumber in under 20 minutes. Incredible service.&rdquo;
        </p>
        <p className="text-white/30 text-xs mt-2">— Adaeze O., Lagos</p>
      </div>
    </div>
  )
}
...
export default function RegisterCustomerPage() {
  ...
  const form2 = useForm<Step2>({ resolver: zodResolver(step2Schema), defaultValues: { state: "" } })
  ...
  const handleVerify = async () => {
    ...
    fd.set("full_name", s2.full_name)
    fd.set("email", s1.email)
    fd.set("phone", s2.phone)
    fd.set("password", s1.password)
    fd.set("state", s2.state)

    const result = await registerCustomerAction(fd)
      ...
      <div className="flex-1 flex flex-col bg-background overflow-y-auto">
        <div className="flex-1 flex flex-col justify-center px-6 py-12 max-w-md w-full mx-auto">
          ...
          <div key={step} style={{ animation: "fade-up 0.4s ease both" }}>
            ...
            {step === 1 && (
              <form onSubmit={goStep2} noValidate className="space-y-5">
                <div className="mb-6">
                  ...
                  <p className="text-muted-foreground text-sm mt-1">Start with your email and a secure password.</p>
                </div>
            ...
            {/* ── Step 2: Profile ──────────────────────────────────────────── */}
            {step === 2 && (
              <form onSubmit={goStep3} noValidate className="space-y-5">
                <div className="mb-6">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="h-11"
                    aria-invalid={!!form2.formState.errors.full_name}
                    {...form2.register("full_name")}
                  />
                  {form2.formState.errors.full_name && (
                    <p className="text-xs text-destructive">{form2.formState.errors.full_name.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => form2.setValue("state", v ?? "", { shouldValidate: true })}
                  >
                  ...
                  {form2.formState.errors.state && (
                    <p className="text-xs text-destructive">{form2.formState.errors.state.message}</p>
                  )}
                </div>
